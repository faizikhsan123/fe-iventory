  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div>
        ...
        <p className="text-sm text-neutral-500">Ringkasan inventaris</p>
      </div>
...
function CardSummary({ icon, iconBg, label, value }: CardSummaryProps) {
  return (
    <div className="rounded-2xl border border-neutral-200 bg-white p-5">
      ...
      <p className="text-2xl font-bold text-neutral-900">{value.toLocaleString("id-ID")}</p>
  ...
  return (
    <div className="rounded-2xl border border-neutral-200 bg-white p-6">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="font-semibold text-neutral-900">Aktivitas Stok Terbaru</h2>
        <button className="rounded-lg border border-neutral-200 px-3 py-1.5 text-sm text-neutral-600 hover:bg-neutral-50">
          Lihat semua
        </button>
      </div>
              ...
              {data.map((item, index) => {
                ...
                return (
                  <tr key={index} className="text-neutral-700">
                    <td className="py-3 pr-3 text-neutral-500">{item.date}</td>
                    <td className="py-3 pr-3 text-blue-600">{item.part_number}</td>
                    <td className="py-3 pr-3 font-medium text-neutral-900">{item.name}</td>
                    ...
                    <td className="py-3 pr-3 text-neutral-500">{item.user_name}</td>
                  </tr>
                );
              })}
            </tbody>
...
function TabelTransaksiTerbaru({ data }: { data: TransaksiTerbaruItem[] }) {
  return (
    <div className="rounded-2xl border border-neutral-200 bg-white p-6">
      <div className="mb-4 flex items-center justify-between">
        <h2 className="font-semibold text-neutral-900">Transaksi Terbaru</h2>
        <button className="rounded-lg border border-neutral-200 px-3 py-1.5 text-sm text-neutral-600 hover:bg-neutral-50">
          Lihat semua
        </button>
      </div>
            ...
            <tbody className="divide-y divide-neutral-50">
              {data.map((trx, index) => (
                <tr key={index} className="text-neutral-700">
                  ...
                  <td className="py-3 pr-3 text-neutral-900">{trx.employe_name}</td>
                  <td className="py-3 pr-3 text-neutral-500">{trx.barang || "—"}</td>
                  <td className="py-3 pr-3 text-neutral-500">{trx.date}</td>
                </tr>
              ))}
            </tbody>
          </table>
